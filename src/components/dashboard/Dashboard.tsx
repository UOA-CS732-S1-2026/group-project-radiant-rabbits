...
function RepoContributionBars({ rows }: { rows: ContributorRow[] }) {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex flex-1 flex-col justify-around gap-sm">
        {rows.map((row) => {
          const total = row.commits + row.prs + row.issues;
          return (
            <div
              key={row.name}
              className="grid grid-cols-[110px_1fr_40px] items-center gap-md"
            >
      ...
      <div className="mt-md flex flex-wrap gap-md text-body-xs text-brand-dark/50 pt-sm">
