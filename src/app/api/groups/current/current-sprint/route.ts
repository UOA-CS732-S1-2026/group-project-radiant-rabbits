...
// CHANGE TO REFRESH BUTTON LATER TO REDUCE DEPENDENCY ON LIVE API DATA ??
...
// Fallback function to fetch data from MongoDB if GitHub API call fails or access token is not available
...
export async function GET() {
  try {
    ...
    let periodActivity: Awaited<ReturnType<typeof getPeriodActivityFromDb>>;
    ...
    return NextResponse.json(
      {
        ...
        sprint: {
          id: computedSprint.id,
          number: computedSprint.number,
          name: computedSprint.name,
          startDate: computedSprint.startDate,
          endDate: computedSprint.endDate,
          status: computedSprint.status,
          progress: computedSprint.progress,
        },
    ...
    return NextResponse.json(
      { error: "Failed to fetch current sprint" },
      { status: 500 },
    );
  }
}
